...
app.layout = dbc.Container([
    html.Div([
        dbc.Row([
            dbc.Col(html.Img(src="/assets/logo_eth.png", style={"height": "60px"}), width="auto"),
            dbc.Col(html.H1("Suivi du prix de l'ETH", style={
                "textAlign": "center", "color": "white",
                "fontFamily": "Montserrat", "padding": "20px 0",
                "marginBottom": "0"
            }), width=True),
            dbc.Col(html.Img(src="/assets/logo_eth.png", style={"height": "60px"}), width="auto")
        ], justify="center", align="center")
    ], style={"backgroundColor": "#0a1a2f", "borderRadius": "10px", "marginBottom": "30px"}),
        ...
        dbc.Col([
            dbc.Card([
                ...
                dbc.CardBody([
                    ...
                    html.Hr(),
                    html.Div(id="insight-stats", style={"color": "lightgray", "fontSize": "13px"})
                ])
...
def update_dashboard(n, start_date, end_date, agg):
    ...
    stats_adv = html.Div([
        html.Div([
            html.Span("Volatilité (ecart-type) : ", style={"color": "gray", "fontWeight": "bold"}), f"{volatility:.4f}"
        ]),
        html.Div([
            html.Span("Prix max : ", style={"color": "gray", "fontWeight": "bold"}), f"{prix_max:.4f}"
        ]),
        html.Div([
            html.Span("Prix min : ", style={"color": "gray", "fontWeight": "bold"}), f"{prix_min:.4f}"
        ]),
        html.Div([
            html.Span("Moyenne : ", style={"color": "gray", "fontWeight": "bold"}), f"{moyenne:.4f}"
        ])
    ])
    ...
    insights = html.Div([
        html.Div(f"Plus forte hausse : {max_rise:.2f}% le {max_rise_day}"),
        html.Div(f"Journée la plus volatile : {most_volatile:.2f} le {most_volatile_day}"),
        html.Div(f"Prix le plus bas : {lowest_price:.2f} le {lowest_price_day}")
    ])
    ...
    for date, row in df_daily.iterrows():
        color = "green" if row["price_last"] > row["price_first"] else "red"
    ...
    return stats_main, price_fig, bars, df["timestamp"].min(), df["timestamp"].max(), stats_adv, insights
