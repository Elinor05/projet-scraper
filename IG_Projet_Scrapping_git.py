...
app.layout = dbc.Container([
    html.Div([
        html.Div([
            html.Img(src="/assets/logo_eth.png", style={"height": "50px", "marginRight": "15px"}),
            ...
            html.Img(src="/assets/logo_eth.png", style={"height": "50px", "marginLeft": "15px"})
        ], style={
            "display": "flex",
...
def update_dashboard(n, start_date, end_date, agg):
    ...
    stats_adv = html.Div([
        html.Div([
            html.Div([
                html.Div([
                    html.Span("Volatilité (écart-type) : ", style={"color": "lightgray", "fontWeight": "bold"}),
                    html.Span(f"{volatility:.4f}")
                ]),
                html.Div([
                    html.Span("Prix max : ", style={"color": "lightgray", "fontWeight": "bold"}),
                    html.Span(f"{prix_max:.4f}")
                ]),
                html.Div([
                    html.Span("Prix min : ", style={"color": "lightgray", "fontWeight": "bold"}),
                    html.Span(f"{prix_min:.4f}")
                ]),
                html.Div([
                    html.Span("Moyenne : ", style={"color": "lightgray", "fontWeight": "bold"}),
                    html.Span(f"{moyenne:.4f}")
                ])
            ], style={"width": "48%", "display": "inline-block", "verticalAlign": "top"}),

            html.Div([
                html.Div([
                    html.Span("Plus forte hausse : ", style={"color": "lightgray", "fontWeight": "bold"}),
                    html.Span(f"{max_rise:.2f}% le {max_rise_day}")
                ]),
                html.Div([
                    html.Span("Journée la plus volatile : ", style={"color": "lightgray", "fontWeight": "bold"}),
                    html.Span(f"{most_volatile:.2f} le {most_volatile_day}")
                ]),
                html.Div([
                    html.Span("Prix le plus bas : ", style={"color": "lightgray", "fontWeight": "bold"}),
                    html.Span(f"{lowest_price:.2f} le {lowest_price_day}")
                ])
            ], style={"width": "48%", "display": "inline-block", "paddingLeft": "15px", "verticalAlign": "top"})
        ])
    ])
